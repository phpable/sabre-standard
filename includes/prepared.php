...
public function h($v){ return is_object($v) && method_exists($v, 'toHtml')
? $v->toHtml() : htmlspecialchars($this->o($v), ENT_QUOTES, "UTF-8", false); }

public function f($v){ return array_filter($v, function ($s){
return !in_array($s, ['GLOBALS', '_POST', '_GET', '_FILES', '_SERVER',
'_COOKIE', '__export', '__data', '__obj']) ; }, ARRAY_FILTER_USE_KEY); }

public function s($n){ static $s = []; if (func_num_args() < 2) { if (isset($s[$n])) { foreach ($s[$n] as $o) {
